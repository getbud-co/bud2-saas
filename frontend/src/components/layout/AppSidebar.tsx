...
export function AppSidebar({
  ...
  const { activeOrganization: configActiveOrganization, setActiveOrgId } =
    useConfigData();
  const {
    logout,
    user: authUser,
  ...
  const sidebarUserName = authUser
    ...
    <>
      ...
      >
        ...
        <SidebarOrgSwitcher
          ref={orgRef}
          image={
            configActiveOrganization?.logoUrl ??
            orgItems.find((org) => org.id === activeOrganization?.id)?.image ??
            orgItems[0]?.image
          }
          label={activeOrganization?.name ?? orgItems[0]?.label ?? ""}
          onClick={() => {
            if (!switchingOrgId) setOrgOpen((v) => !v);
          }}
        />
        <Popover
          items={orgMenuItems}
        ...
        <SidebarNav>
          ...
          <SidebarGroup label="Performance e Engajamento">
            ...
            >
              ...
              <SidebarSubItem
                onClick={() =>
                  navigate("/missions", { state: { newView: true } })
                }
              >
                + Criar visualização
              </SidebarSubItem>
            ...
            >
              <SidebarSubItem
                active={
                  location.pathname === "/surveys" && !searchParams.get("view")
                }
              ...
              <SidebarSubItem
                onClick={() =>
                  navigate("/surveys", { state: { newView: true } })
                }
              >
                + Criar visualização
              </SidebarSubItem>

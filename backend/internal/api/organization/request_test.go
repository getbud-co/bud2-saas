...
func TestCreateRequestToCommand(t *testing.T) {
	req := createRequest{Name: "Test Org", Domain: "example.com", Workspace: "example", Status: "active"}

	cmd := req.toCommand()

	assert.Equal(t, "Test Org", cmd.Name)
	assert.Equal(t, "example.com", cmd.Domain)
...
func TestUpdateRequestToCommand(t *testing.T) {
	...
	req := updateRequest{Name: "Updated Org", Domain: "example.com", Workspace: "example", Status: "inactive"}

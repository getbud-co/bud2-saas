...
type createUseCase interface {
	Execute(ctx context.Context, cmd appteam.CreateCommand) (*domainteam.Team, error)
}

type getUseCase interface {
	Execute(ctx context.Context, organizationID domain.TenantID, id uuid.UUID) (*domainteam.Team, error)
}

type listUseCase interface {
	Execute(ctx context.Context, cmd appteam.ListCommand) (domainteam.ListResult, error)
}

type updateUseCase interface {
	Execute(ctx context.Context, cmd appteam.UpdateCommand) (*domainteam.Team, error)
}

type deleteUseCase interface {
	Execute(ctx context.Context, cmd appteam.DeleteCommand) error
}
...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Location", fmt.Sprintf("/teams/%s", result.ID))
...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	cmd := appteam.ListCommand{OrganizationID: organizationID, Page: page, Size: size}
	if s := q.Get("status"); s != "" {
		cmd.Status = &s
	}
	...
	items := make([]Response, len(result.Teams))
	for i := range result.Teams {
		items[i] = toResponse(&result.Teams[i])
	}
...
func (h *Handler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.delete.Execute(r.Context(), appteam.DeleteCommand{OrganizationID: organizationID, ID: id}); err != nil {

...
func TestHandleError_MapsDomainErrorsToHTTPStatuses(t *testing.T) {
	cases := []struct {
		...
		{"ErrNotFound → 404", domainindicator.ErrNotFound, http.StatusNotFound},
		{"ErrInvalidReference → 422", domainindicator.ErrInvalidReference, http.StatusUnprocessableEntity},
...
func TestHandleError_WrappedErrors_StillMatchByErrorsIs(t *testing.T) {
	wrapped := indicatorErrWrap{inner: domainindicator.ErrInvalidReference}
	rr := httptest.NewRecorder()
...
type indicatorErrWrap struct{ inner error }

func (e indicatorErrWrap) Error() string { return "wrap: " + e.inner.Error() }
func (e indicatorErrWrap) Unwrap() error { return e.inner }

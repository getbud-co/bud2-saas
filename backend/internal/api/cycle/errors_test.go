...
func TestHandleError_MapsDomainErrorsToHTTPStatuses(t *testing.T) {
	cases := []struct {
		...
		{"ErrNotFound → 404", domaincycle.ErrNotFound, http.StatusNotFound},
		{"ErrNameExists → 409", domaincycle.ErrNameExists, http.StatusConflict},
...
func TestHandleError_WrappedErrors_StillMatchByErrorsIs(t *testing.T) {
	wrapped := cycleErrWrap{inner: domaincycle.ErrNameExists}
	rr := httptest.NewRecorder()
...
type cycleErrWrap struct{ inner error }

func (e cycleErrWrap) Error() string { return "wrap: " + e.inner.Error() }
func (e cycleErrWrap) Unwrap() error { return e.inner }
